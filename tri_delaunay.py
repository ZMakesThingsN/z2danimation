...
#Blender whined about this
bl_info	= {
	"name":	"GreaserX Voronoi Module FAKE"
	,"description": ""
	,"author": ""
	,"version": (0, 4, 8)#(2019, 421)
	,"blender": (2, 80, 0)
	#,"location": "Properties space > Scene tab > UVX Anim panel"
	,"wiki_url":	""
	,"category": "3D View"
}
...
class RegionatedTriangleIterator(object):
	...
	def next(self):
		while self.to_visit_stack or self.later:
			...
			if self.later:
				self.group += 1
				for _ in range(len(self.later)):#xrange(len(self.later)):
					t, d = self.later.pop()
